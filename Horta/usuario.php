...
Class Usuario{
    ...
    public function queryOne(array $parameters = []){
        $resultado = $this->conexao->executeQuery("SELECT * FROM usuario WHERE id = :id", $parameters);
    ...
    public function atualizaCadastro(array $parameters = [], array $FILE = []){
        ...
        if (move_uploaded_file($FILE["fotoUsuario"]["tmp_name"],$_UP["pasta"].$fotoUsuario)) {
            ...
            $sql = "UPDATE usuario SET nome = :nome, fotoUsuario = :fotoUsuario, email = :email, senha = :senha WHERE id = 1";
