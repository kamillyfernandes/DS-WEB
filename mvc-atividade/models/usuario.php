...
Class Usuario{
    ...
    public function atualizaCadastro(array $parameters = [], array $FILE = []){ 
        //Cria as variaveis de configuração e validação, verificando  cada funçao
        $_UP["pasta"] = 'photos/';
        ...
        if ($FILE["fotoUsuario"]["error"] != 0) { //se foto usuario for diferente de 0 print o erro 
        ...
        if (move_uploaded_file($FILE["fotoUsuario"]["tmp_name"],$_UP["pasta"].$fotoUsuario)) { // se passou por toda validação vai moder o arquico tmp name para nss pasta   
            $parameters[':fotoUsuario'] = $fotoUsuario; //parametros= todas array de cadastro, e adicionamos a foto do usaurioo
            ...
            $resultado = $this->conexao->executeQuery($sql, $parameters); //executando a query com sql e parameters
